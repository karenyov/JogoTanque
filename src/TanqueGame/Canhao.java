...
public class Canhao implements ImageObserver {
	private double x,y;
	private double angulo;
	private double velocidadeCanhao;
	private Color cor;
	
	public Canhao (double x, double y, double angulo, Color cor){
		this.x = x;
		this.y = y;
		this.angulo = angulo;
		this.cor = cor;
		this.velocidadeCanhao = 20;
	}   
	...
	public void mover(){
		x = x + Math.sin(Math.toRadians(angulo)) * velocidadeCanhao;
		y = y - Math.cos(Math.toRadians(angulo)) * velocidadeCanhao;
		
		
	}
	
	public void draw(Graphics2D g2d) throws IOException{
		BufferedImage bomba = ImageIO.read(new File("bomba.png"));
		...
		//Desenhando a bala
		g2d.drawImage(bomba, 5, 5, 20, 20, this);
